...
def lambda_handler(event, context):
    ...
    evaluationResourceTags = AWS_EC2_CLIENT.describe_tags(
        Filters=[
            {
                'Name': 'resource-id',
                'Values': [
                    evaluation_copy[0]['ComplianceResourceId'],
                ]
            },
        ])
    
    if evaluationResourceTags['Tags'] != []:
        for tag in evaluationResourceTags['Tags']:
            if tag['Key'] == 'excludeConfigEvaluation':
                return evaluations
