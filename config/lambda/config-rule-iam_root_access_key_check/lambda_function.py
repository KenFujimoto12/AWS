...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    if acc_summary['SummaryMap']['AccountAccessKeysPresent'] == 0:
        return build_evaluation(event['accountId'], 'COMPLIANT', event)

    return build_evaluation(event['accountId'], 'NON_COMPLIANT', event, annotation='The root user has access key(s).')
